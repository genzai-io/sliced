...
func testFastLogStore(t testing.TB, inmem bool) *LogStore {
	...
	store, err := NewLogStore(name, Medium, moved.Logger)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestFastLogStore_Implements(t *testing.T) {
	...
	//	t.Fatalf("LogStore does not implement raft.PeerStore")
	//}
}

func TestFastLogStore_Shrink(t *testing.T) {
	store, err := NewLogStore("/Users/clay/go/src/github.com/tidwall/summitdb/data3/raft.db", High, moved.Logger)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestNewFastLogStore(t *testing.T) {
	...
	// Successfully creates and returns a store
	store, err := NewLogStore(fh.Name(), High, moved.Logger)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestFastLogStore_Persist(t *testing.T) {
	for p := 0; p < 2; p++ {
		...
		store.Close()
		var err error
		store, err = NewLogStore(store.path, Medium, moved.Logger)
		if err != nil {
			t.Fatal(err)
		}
	}
